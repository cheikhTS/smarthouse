 ...
 * @author Florent
 */
public class EquipmentSerializationDAO extends DAO<Equipment> {
	...
	public Equipment find(int id) {
				...
				for ( Room room : area.getRooms() ) {
					for ( int i = 0; i < room.getEquipments().size(); i++ ) {
						if ( room.getEquipments().get(i).getId() == id ) {
							logger.debug(this.getClass().getName() + " - Eqp search [id=" + id + "] succeed");
		...
		logger.debug(this.getClass().getName() + " - Eqp search [id=" + id + "] failed");
		return null;
	}
