...
public class ModTriggerGUI extends JPanel implements ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -1451595757170853768L;
	protected ATrigger trigger = null;
	 ...
	 * Create the panel.
	 */
	public ModTriggerGUI (ATrigger _trigger) {
		trigger = _trigger;
		initComponents();
	}
	...
	private void initComponents() {
		...
		lblAjouter = new JLabel("Modifier un déclencheur");
		add(lblAjouter);
		...
		jdc_selectDate = new JDateChooser(trigger.getDateStart());
		...
		if (null != trigger.getDateExpiration()) {
			jdc_selectDateExpiration = new JDateChooser(trigger.getDateExpiration());
			chckb_useExpirDate.setSelected(true);
			jdc_selectDateExpiration.setEnabled(true);
		} else {
		...
		ftf_selectStartTime = new JFormattedTextField(trigger.getStartTime());
	...
	public ATrigger createTrigger() {
		...
		switch ( cb_selectTrigger.getSelectedIndex() ) {
			case 0:
				trig = new OnceTrigger(trigger.getTask(), jdc_selectDate.getDate(), getTimeInSecond());
			break;
			case 1:
				trig = new DailyTrigger(trigger.getTask(), jdc_selectDate.getDate(), getTimeInSecond(), dailyTriggerGui.getNbofRepeat());
			break;
			case 2:
				trig = new WeeklyTrigger(trigger.getTask(), jdc_selectDate.getDate(), getTimeInSecond(), weeklyTriggerGui.getSelectValidDay(), weeklyTriggerGui.getNbofRepeat());
			break;
			case 3:
				trig = new MonthlyTrigger(trigger.getTask(), jdc_selectDate.getDate(), getTimeInSecond(), monthlyTriggerGui.getSelectValidMonth(), monthlyTriggerGui.getSelectValidNumberDay());
			break;
		}
	...
	public void actionPerformed(ActionEvent e) {
		if ( e.getSource() == cb_selectTrigger ) {
			...
			Scenario scenario = trigger.getTask().getScenario();
	...
	private void deleteTrigger() {
		Scenario scenario = trigger.getTask().getScenario();
		trigger.getTask().getTriggers().remove(trigger);
